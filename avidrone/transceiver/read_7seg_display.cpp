...
void int_a_dir(void) {
    if ((interrupt_count != -1) && (interrupt_count < NUM_READS) && (SETUP_COMPLETE == true)) {
...
// Takes bool array of length 5 representing the direction LEDs formatted as such,
// {Leftmost, Left, Center, Right, Rightmost} Returns a double from 0-4 where zero is 
// returned when the leftmost led is on and 4 is returned when the rightmost LED is on
...
// Uses char dig[3] and int dir and returns array of doubles with direction then distance
...
void get_dir_dig(double *dir_dig_out) {
    ...
    for (int i = 0; i < NUM_READS; i++) {
        all_dig_decode[i][0] = decode_7seg((bool*)(all_dig1_reading[i]));
        all_dig_decode[i][2] = decode_7seg((bool*)(all_dig2_reading[i]));
        ...
        all_dir_dig_out[i][0] = dir_decode((bool*)(all_dir_reading[i]));
    }
