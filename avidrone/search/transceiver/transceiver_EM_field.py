...
# coil 1
ts = np.linspace(-16, 16, 1000)
vertices_1 = np.c_[5 * np.cos(ts * 2 * np.pi), 5 * np.sin(ts * 2 * np.pi), ts]

coil_1 = magpy.current.Line(current=50, vertices=vertices_1)
...
coil_2 = magpy.current.Line(current=50, vertices=vertices_2)
