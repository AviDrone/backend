...
parser = argparse.ArgumentParser(
    description="Load a telemetry log and use position data to create mission waypoints for a vehicle. Connects to SITL on local PC by default."
)
parser.add_argument("--connect", help="vehicle connection target.")
parser.add_argument(
    "--tlog", default="flight.tlog", help="Telemetry log containing path to replay"
)
...
def position_messages_from_tlog(filename):
    ...
    while True:
        ...
        if pt2num == num_points - 1 or len(kept_messages) == 99:
            ...
            break
        pt1 = LocationGlobalRelative(
            messages[pt1num].lat / 1.0e7, messages[pt1num].lon / 1.0e7, 0
        )
        pt2 = LocationGlobalRelative(
            messages[pt2num].lat / 1.0e7, messages[pt2num].lon / 1.0e7, 0
        )
...
def arm_and_takeoff(aTargetAltitude):
    ...
    while True:
        ...
        print(
            " Altitude: %f < %f"
            % (vehicle.location.global_relative_frame.alt, requiredAlt)
        )
        time.sleep(1)
